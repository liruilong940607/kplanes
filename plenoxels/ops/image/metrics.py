...
from .io import write_png, write_video_to_file
...
def msssim(rgb, gts):
    ...
    assert gts.max() <= 1.05 and gts.min() >= -0.05
    return ms_ssim(
        torch.permute(rgb[None, ...], (0, 3, 1, 2)),
        torch.permute(gts[None, ...], (0, 3, 1, 2)),
    ).item()
...
def flip(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i in range(len(pred_frames)):
            ...
            all_results.append(
                extract_from_result(result, r"Mean: (\d+\.\d+)")
            )
