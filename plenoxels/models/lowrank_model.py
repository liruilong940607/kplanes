...
from plenoxels.raymarching.spatial_distortions import (
    SceneContraction,
    SpatialDistortion,
)
...
class LowrankModel(nn.Module):
        ...
        **kwargs,
        ...
        if use_same_proposal_network:
            ...
            for i in range(self.num_proposal_iterations):
                prop_net_args = self.proposal_net_args_list[
                    min(i, len(self.proposal_net_args_list) - 1)
                ]
                network = KPlaneDensityField(
                    aabb,
                    spatial_distortion=self.spatial_distortion,
    ...
    @staticmethod
    def render_depth(
        weights: torch.Tensor, ray_samples: RaySamples, rays_d: torch.Tensor
    ):
    ...
    def forward(
        self, rays_o, rays_d, bg_color, near_far: torch.Tensor, timestamps=None
    ):
        """
        rays_o : [batch, 3]
        rays_d : [batch, 3]
