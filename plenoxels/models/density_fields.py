...
class KPlaneDensityField(nn.Module):
    def __init__(
        ...
        self.grids = init_grid_param(
            grid_nd=2,
            in_dim=num_input_coords,
            out_dim=num_output_coords,
            reso=resolution,
            a=0.1,
            b=0.15,
        )
        ...
        log.info(
            f"Initialized KPlaneDensityField. hexplane={self.hexplane} - "
    ...
    def get_params(self):
        ...
        nn_params = {
            k: v
            for k, v in self.sigma_net.named_parameters(prefix="sigma_net")
        }
        other_params = {
            k: v
