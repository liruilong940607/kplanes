    ...
    def eval_step(self, data, **kwargs) -> MutableMapping[str, torch.Tensor]:
        ...
        with torch.cuda.amp.autocast(enabled=self.train_fp16), torch.no_grad():
            ...
            for b in range(math.ceil(rays_o.shape[0] / batch_size)):
                ...
                timestamps_d_b = timestamp.expand(rays_o_b.shape[0]).to(
                    self.device
                )
                outputs = self.model(
                    rays_o_b,
                    rays_d_b,
                    timestamps=timestamps_d_b,
                    bg_color=bg_color,
                    near_far=near_far,
                )
    ...
    def validate(self):
        ...
        if self.save_video:
            ...
            if len(out_depths) > 0:
                write_video_to_file(
                    os.path.join(
                        self.log_dir, f"step{self.global_step}-depth.mp4"
                    ),
                    out_depths,
                )
        ...
        if self.compute_video_metrics:
            per_scene_metrics["JOD"] = metrics.jod(
                ...
                [
                    f[dataset.img_h : 2 * dataset.img_h, :, :]
...
def init_tr_data(data_downsample, data_dir, **kwargs):
    ...
    tr_dset = Video360Dataset(
        ...
        ndc=kwargs["ndc"],
        near_scaling=float(kwargs.get("near_scaling", 0)),
        ndc_far=float(kwargs.get("ndc_far", 0)),
...
def init_ts_data(data_dir, split, **kwargs):
    ...
    ts_dset = Video360Dataset(
        ...
        max_cameras=kwargs.get("max_test_cameras", None),
        max_tsteps=kwargs.get("max_test_tsteps", None),
        contraction=kwargs["contract"],
        ndc=kwargs["ndc"],
        near_scaling=float(kwargs.get("near_scaling", 0)),
        ndc_far=float(kwargs.get("ndc_far", 0)),
