...
class BaseTrainer(abc.ABC):
    ...
    def train_step(self, data, **kwargs) -> bool:
        ...
        if self.global_step % self.calc_metrics_every == 0:
            with torch.no_grad():
                ...
                self.loss_info[f"psnr"].update(
                    -10 * math.log10(recon_loss_val)
                )
                for r in self.regularizers:
    ...
    def _normalize_err(
        self, preds: torch.Tensor, gt: torch.Tensor
    ) -> torch.Tensor:
    ...
    def _normalize_depth(
        self, depth: torch.Tensor, img_h: int, img_w: int
    ) -> torch.Tensor:
        return (
            (self._normalize_01(depth)).cpu().reshape(img_h, img_w)[..., None]
        )
    ...
    def evaluate_metrics(
        ...
        preds_rgb = preds["rgb"].reshape(img_h, img_w, 3).cpu().clamp(0, 1)
        if not torch.isfinite(preds_rgb).all():
            log.warning(
                f"Predictions have {torch.isnan(preds_rgb).sum()} NaNs, "
        ...
        for k in preds.keys():
            if "depth" in k:
                ...
                out_depth = (
                    torch.cat((out_depth, prop_depth))
        ...
        if gt is not None:
            ...
            out_img = torch.cat(
                (out_img, self._normalize_err(preds_rgb, gt)), dim=0
            )
        ...
        if save_outputs:
            ...
            if name is not None and name != "":
                out_name += "-" + name
            write_png(
                os.path.join(self.log_dir, out_name + ".png"), out_img_np
            )
            if out_depth is not None:
                depth_name = out_name + "-depth"
                write_png(
                    os.path.join(self.log_dir, depth_name + ".png"),
                    out_depth_np,
                )
    ...
    def report_test_metrics(
        self,
        scene_metrics: Dict[str, Sequence[float]],
    ...
    def init_lr_scheduler(
        ...
        log.info(
            f"Initializing LR Scheduler of type {scheduler_type} with "
    ...
    def init_regularizers(self, **kwargs):
        ...
        regularizers = [
            r for r in self.get_regularizers(**kwargs) if r.weight > 0
        ]
        return regularizers
...
def initialize_model(
    ...
    **kwargs,
    ...
    log.info(
        f"Initialized {model.__class__} model with "
