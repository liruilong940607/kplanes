...
from typing import Any, Dict, MutableMapping, Sequence, Union

import pandas as pd
import torch
import torch.utils.data

from plenoxels.datasets import LLFFDataset, SyntheticNerfDataset
    ...
    def eval_step(self, data, **kwargs) -> MutableMapping[str, torch.Tensor]:
        ...
        with torch.cuda.amp.autocast(enabled=self.train_fp16), torch.no_grad():
            ...
            for b in range(math.ceil(rays_o.shape[0] / batch_size)):
                ...
                outputs = self.model(
                    rays_o_b, rays_d_b, near_far=near_far, bg_color=bg_color
                )
...
def decide_dset_type(dd) -> str:
    if (
        ...
        raise RuntimeError(
            f"data_dir {dd} not recognized as LLFF or Synthetic dataset."
        )
...
def init_tr_data(data_downsample: float, data_dirs: Sequence[str], **kwargs):
    ...
    if dset_type == "synthetic":
        ...
        dset = LLFFDataset(
            data_dir,
            split="train",
            downsample=int(data_downsample),
            hold_every=hold_every,
            batch_size=batch_size,
            contraction=kwargs["contract"],
            ndc=kwargs["ndc"],
            ndc_far=float(kwargs["ndc_far"]),
            near_scaling=float(kwargs["near_scaling"]),
        )
    else:
