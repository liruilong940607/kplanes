...
class PlaneTV(Regularizer):
    def __init__(self, initial_value, what: str = "field"):
        if what not in {"field", "proposal_network"}:
            raise ValueError(
                f'what must be one of "field" or "proposal_network" '
...
class HistogramLoss(Regularizer):
    ...
    def _regularize(
        self, model: LowrankModel, model_out, **kwargs
    ) -> torch.Tensor:
        if self.visualize:
            if self.count % 500 == 0:
                prop_idx = 0
                ...
                # proposal info
                weights_proposal = (
                    model_out["weights_list"][prop_idx].detach().cpu().numpy()
                )
                spacing_starts_proposal = model_out["ray_samples_list"][
                    prop_idx
                ].spacing_starts
                spacing_ends_proposal = model_out["ray_samples_list"][
                    prop_idx
                ].spacing_ends
                sdist_proposal = (
                    torch.cat(
                ...
                # fine info
                weights_fine = (
                    model_out["weights_list"][fine_idx].detach().cpu().numpy()
                )
                spacing_starts_fine = model_out["ray_samples_list"][
                    fine_idx
                ].spacing_starts
                spacing_ends_fine = model_out["ray_samples_list"][
                    fine_idx
                ].spacing_ends
                sdist_fine = (
                    torch.cat(
                ...
                for i in range(10):  # plot 10 rays
                    ...
                    ax1.bar(
                        sdist_proposal[i, :-1],
                        weights_proposal[i].squeeze() / delta,
                        width=delta,
                        align="edge",
                        label="proposal",
                        alpha=0.7,
                    ...
                    plt.clf()
            self.count += 1
        return interlevel_loss(
            model_out["weights_list"], model_out["ray_samples_list"]
        )
