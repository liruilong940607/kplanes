...
class PhototourismTrainer(BaseTrainer):
    ...
    def eval_step(self, data, **kwargs) -> MutableMapping[str, torch.Tensor]:
        ...
        with torch.cuda.amp.autocast(enabled=self.train_fp16), torch.no_grad():
            ...
            for b in range(math.ceil(rays_o.shape[0] / batch_size)):
                rays_o_b = rays_o[b * batch_size : (b + 1) * batch_size].to(
                    self.device
                )
                rays_d_b = rays_d[b * batch_size : (b + 1) * batch_size].to(
                    self.device
                )
                timestamps_b = timestamp[
    ...
    def validate(self):
        ...
        for img_idx, data in enumerate(dataset):
            ...
            pb.set_postfix_str(
                f"PSNR={out_metrics['psnr']:.2f}", refresh=False
            )
            pb.update(1)
        pb.close()
        val_metrics = [
        ...
        df.to_csv(
            os.path.join(
                self.log_dir, f"test_metrics_step{self.global_step}.csv"
            )
        )
    ...
    def optimize_appearance_step(self, data, im_id):
        ...
        app_optim = torch.optim.Adam(
            params=self.model.field.test_appearance_embedding.parameters(),
            lr=self.extra_args["app_optim_lr"],
        )
        lr_sched = torch.optim.lr_scheduler.StepLR(
            app_optim, step_size=3 * n_steps, gamma=0.1
        )
        ...
        for n in range(epochs):
            ...
            for b in range(n_steps):
                ...
                self.writer.add_scalar(
                    f"appearance_loss_{self.global_step}/recon_loss_{im_id}",
                    recon_loss.item(),
                    b + n * n_steps,
                )
    ...
    def optimize_appearance_codes(self):
        ...
        if not hasattr(self.model.field, "test_appearance_embedding"):
            ...
            with torch.autograd.no_grad():
                tst_embedding.weight.copy_(
                    self.model.field.appearance_embedding.weight.detach()
        ...
        # 3. Optimize
        pb = tqdm(
            total=len(dset), desc=f"Test-time appearance-code optimization"
        )
...
def load_data(
    data_downsample, data_dirs, validate_only, render_only, **kwargs
):
