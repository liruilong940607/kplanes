...
from .synthetic_nerf_dataset import load_360_images, load_360_intrinsics
    ...
    def __init__(
        ...
        if dset_type == "llff":
            if split == "render":
                assert (
                    ndc
                ), "Unable to generate render poses without ndc: don't know near-far."
                (
                    per_cam_poses,
                    per_cam_near_fars,
                    intrinsics,
                    _,
                ) = load_llffvideo_poses(
                    datadir,
                ...
                if contraction:
                    ...
                    self.per_cam_near_fars = torch.tensor(
                        [[0.0, self.ndc_far]]
                    ).repeat(per_cam_near_fars.shape[0], 1)
            # These values are tuned for the salmon video
            self.global_translation = torch.tensor([0, 0, 2.0])
            ...
            assert (
                not contraction
            ), "Synthetic video dataset does not work with contraction."
            ...
            if split == "render":
                ...
                render_poses = torch.stack(
                    [
                        ...
                        for angle in np.linspace(-180, 180, num_tsteps + 1)[
                            :-1
                        ]
                    ],
                    0,
                ...
                imgs, self.poses = load_360_images(
                    frames, datadir, split, self.downsample
                )
                timestamps = torch.tensor(
                    [fetch_360vid_info(f)[0] for f in frames],
                    dtype=torch.float32,
                )
            ...
            if "dnerf" in datadir:
                ...
                timestamps = (
                    timestamps.float() / torch.amax(timestamps)
                ) * 2 - 1
        ...
        if split == "train":
            self.timestamps = (
                ...
                .repeat(1, intrinsics.height, intrinsics.width)
                .reshape(-1)
            )  # [n_frames * h * w]
        assert (
            self.timestamps.min() >= -1.0 and self.timestamps.max() <= 1.0
        ), "timestamps out of range."
        if imgs is not None and imgs.dtype != torch.uint8:
            imgs = (imgs * 255).to(torch.uint8)
        if (
            self.median_imgs is not None
            and self.median_imgs.dtype != torch.uint8
        ):
        ...
        if (
            split == "train" and dset_type == "llff"
        ):  # Only use importance sampling with DyNeRF videos
            if os.path.exists(os.path.join(datadir, f"isg_weights.pt")):
                ...
                self.isg_weights = dynerf_isg_weight(
                    ...
                    median_imgs=self.median_imgs,
                    gamma=gamma,
                )
                # Normalize into a probability distribution, to speed up sampling
                self.isg_weights = self.isg_weights.reshape(-1) / torch.sum(
                    self.isg_weights
                )
                torch.save(
                    self.isg_weights, os.path.join(datadir, f"isg_weights.pt")
                )
                t_e = time.time()
                log.info(
            ...
            if os.path.exists(os.path.join(datadir, f"ist_weights.pt")):
                ...
                # Normalize into a probability distribution, to speed up sampling
                self.ist_weights = self.ist_weights.reshape(-1) / torch.sum(
                    self.ist_weights
                )
                torch.save(
                    self.ist_weights, os.path.join(datadir, f"ist_weights.pt")
                )
                t_e = time.time()
                log.info(
        ...
        log.info(
            f"VideoDataset contracted={self.is_contracted}, ndc={self.is_ndc}. "
    ...
    def __getitem__(self, index):
        ...
        if self.split == "train":
            ...
            if self.weights_subsampled == 1 or self.sampling_weights is None:
                ...
                hsub, wsub = (
                    h // self.weights_subsampled,
                    w // self.weights_subsampled,
                )
                image_id = torch.div(index, hsub * wsub, rounding_mode="floor")
                ysub = torch.remainder(index, hsub * wsub).div(
                    wsub, rounding_mode="floor"
                )
        ...
        if self.split == "train":
            num_frames_per_camera = len(self.imgs) // (
                len(self.per_cam_near_fars) * h * w
            )
            camera_id = torch.div(
                image_id, num_frames_per_camera, rounding_mode="floor"
            )  # (num_rays)
...
def get_bbox(
    datadir: str, dset_type: str, is_contracted=False
) -> torch.Tensor:
...
def load_360video_frames(
    datadir, split, max_cameras: int, max_tsteps: Optional[int]
) -> Tuple[Any, Any]:
    ...
    if max_tsteps is not None:
        ...
        subsample_time = int(
            math.floor(len(timestamps) / (num_timestamps - 1))
        )
...
def load_llffvideo_poses(
    datadir: str, downsample: float, split: str, near_scaling: float
    ...
    videopaths = np.array(
        glob.glob(os.path.join(datadir, "*.mp4"))
    )  # [n_cameras]
...
def load_llffvideo_data(
    ...
    if keyframes and (keyframes_take_each is None or keyframes_take_each < 1):
        raise ValueError(
            f"'keyframes_take_each' must be a positive number, "
...
def dynerf_isg_weight(imgs, median_imgs, gamma):
    ...
    squarediff = (
        ...
        .div_(255.0)
        .sub_(median_imgs[:, None, ...].float().div_(255.0))
        .square_()  # noqa
...
@torch.no_grad()
def dynerf_ist_weight(
    imgs, num_cameras, alpha=0.1, frame_shift=25
):  # DyNerf uses alpha=0.1
    assert imgs.dtype == torch.uint8
    N, h, w, c = imgs.shape
    frames = imgs.view(
        num_cameras, -1, h, w, c
    ).float()  # [num_cameras, num_timesteps, h, w, 3]
    max_diff = None
    shifts = list(range(frame_shift + 1))[1:]
    for shift in shifts:
        shift_left = torch.cat(
            [frames[:, shift:, ...], torch.zeros(num_cameras, shift, h, w, c)],
            dim=1,
        )
        shift_right = torch.cat(
            [
                torch.zeros(num_cameras, shift, h, w, c),
                frames[:, :-shift, ...],
            ],
            dim=1,
        )
        mymax = torch.maximum(
            torch.abs_(shift_left - frames), torch.abs_(shift_right - frames)
        )
        if max_diff is None:
            max_diff = mymax
        else:
