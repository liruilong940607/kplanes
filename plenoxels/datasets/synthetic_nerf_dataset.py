...
class SyntheticNerfDataset(BaseDataset):
    def __init__(
        ...
        if split == "render":
            frames, transform = load_360_frames(
                datadir, "test", self.max_frames
            )
            imgs, poses = load_360_images(
                frames, datadir, "test", self.downsample
            )
        ...
        log.info(
            f"SyntheticNerfDataset. Loaded {split} set from {datadir}."
    ...
    def __getitem__(self, index):
        ...
        if self.split == "train":
            bg_color = torch.rand(
                (1, 3), dtype=pixels.dtype, device=pixels.device
            )
        else:
            if pixels is None:
                bg_color = torch.ones(
                    (1, 3), dtype=torch.float32, device="cuda:0"
                )
            else:
                bg_color = torch.ones(
                    (1, 3), dtype=pixels.dtype, device=pixels.device
        ...
        if pixels is not None:
            pixels = pixels[:, :3] * pixels[:, 3:] + bg_color * (
                1.0 - pixels[:, 3:]
            )
        out["imgs"] = pixels
...
def create_360_rays(
    ...
    for i in range(num_frames):
        rays_o, rays_d = get_rays(
            directions, poses[i], ndc=False, normalize_rd=True
        )  # h*w, 3
        all_rays_o.append(rays_o)
        all_rays_d.append(rays_d)

    all_rays_o = torch.cat(all_rays_o, 0).to(
        dtype=torch.float32
    )  # [n_frames * h * w, 3]
    all_rays_d = torch.cat(all_rays_d, 0).to(
        dtype=torch.float32
    )  # [n_frames * h * w, 3]
    if imgs is not None:
        imgs = imgs.view(-1, imgs.shape[-1]).to(
            dtype=torch.float32
        )  # [N*H*W, 3/4]
    if not merge_all:
...
def load_360_frames(datadir, split, max_frames: int) -> Tuple[Any, Any]:
    with open(os.path.join(datadir, f"transforms_{split}.json"), "r") as f:
        ...
        if split == "train" or split == "test":
            ...
            if subsample > 1:
                log.info(
                    f"Subsampling {split} set to 1 every {subsample} images."
                )
        else:
...
def load_360_images(
    frames, datadir, split, downsample
...
def load_360_intrinsics(transform, img_h, img_w, downsample) -> Intrinsics:
    ...
    if "fl_x" in transform or "fl_y" in transform:
        ...
        fl_x = (
            width / (2 * np.tan(transform["camera_angle_x"] / 2))
            if "camera_angle_x" in transform
            else None
        )
        fl_y = (
            height / (2 * np.tan(transform["camera_angle_y"] / 2))
